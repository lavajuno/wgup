...
_FMT_PEERS = "{name:15} : {cidr4:14} : {cidr6}"
_FMT_ATTRS = "{:20} : {}"
...
class CLI:
        ...
        def create(args: argparse.Namespace):
            ...
            if args.interactive:
                ...
                if c.interfaces.get(iface_name):
                    print("An interface with this name already exists! Please choose a different name.")
                    return 1
                ...
                if not host:
                    ...
                    return 1
            iface = Interface.create(vpn_iface=iface_name, vpn_cidr4=cidr4, vpn_cidr6=cidr6, host=host, port=port)
    ...
    def get_parser():
        ...
        # interface.create
        interface_create = interface_sub.add_parser("create", help="Create an interface")
        ...
        # interface.show
        interface_show = interface_sub.add_parser("show", help="Show details for an interface")
        ...
        # interface.remove
        interface_remove = interface_sub.add_parser("remove", help="Remove an interface")
        ...
        # interface.down
        interface_down = interface_sub.add_parser("down", help="Bring an interface down")

        # interface.enable
        interface_enable = interface_sub.add_parser("enable", help="Enable the systemd target for an interface")

        # interface.disable
        interface_disable = interface_sub.add_parser("disable", help="Disable the systemd target for an interface")

        # interface.restart
        interface_restart = interface_sub.add_parser("restart", help="Restart an interface managed by systemd")

        # interface.reload
        interface_reload = interface_sub.add_parser("reload", help="Reload an interface's config with systemd")
